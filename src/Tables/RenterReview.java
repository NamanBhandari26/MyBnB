...
public class RenterReview {
    ...
    public RenterReview(Connection connection, int HostUID, int RenterUID, int rating, String comment) {
    ...
    public void insert() {
        try (PreparedStatement statement = connection.prepareStatement("INSERT INTO RenterReview (HostUID, RenterUID, rating, comment) VALUES (?, ?, ?, ?)")) {
    ...
    public static RenterReview selectById(Connection connection, int HostUID, int RenterUID) {
        ...
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM RenterReview WHERE HostUID = ? AND RenterUID = ?")) {
    ...
    // Update operation
    public void update() {
        try (PreparedStatement statement = connection.prepareStatement("UPDATE RenterReview SET rating = ?, comment = ? WHERE HostUID = ? AND RenterUID = ?")) {
    ...
    // Delete operation
    public void delete() {
        try (PreparedStatement statement = connection.prepareStatement("DELETE FROM RenterReview WHERE HostUID = ? AND RenterUID = ?")) {
            statement.setInt(1, this.HostUID);
            statement.setInt(2, this.RenterUID);
            statement.executeUpdate();
        } catch (SQLException e) {
    ...
    public static void createTable(Connection connection) {
        try (Statement statement = connection.createStatement()) {
            String sql = "CREATE TABLE IF NOT EXISTS RenterReview (" +
                    "HostUID INT NOT NULL," +
                    "RenterUID INT NOT NULL," +
                    "rating INT NOT NULL," +
                    "comment VARCHAR(255)," +
                    "PRIMARY KEY (HostUID, RenterUID)," +
                    "FOREIGN KEY (HostUID) REFERENCES User(UID)," +
                    "FOREIGN KEY (RenterUID) REFERENCES User(UID))"; // "User" should be replaced with the actual table name of the User table
