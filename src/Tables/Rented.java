...
public class Rented {
    ...
    public Rented(Connection connection, int LID, int UID, double Total, String StartDate, String EndDate, boolean isCanceled) {
    ...
    public void insert() {
        try (PreparedStatement statement = connection.prepareStatement("INSERT INTO Rented (LID, UID, Total, StartDate, EndDate, isCanceled) VALUES (?, ?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    // Update operation
    public void update() {
        try (PreparedStatement statement = connection.prepareStatement("UPDATE Rented SET LID = ?, UID = ?, Total = ?, StartDate = ?, EndDate = ?, isCanceled = ? WHERE RID = ?")) {
            statement.setInt(1, this.LID);
            statement.setInt(2, this.UID);
            statement.setDouble(3, this.Total);
            statement.setString(4, this.StartDate);
    ...
    public static void createTable(Connection connection) {
        try (Statement statement = connection.createStatement()) {
            String sql = "CREATE TABLE IF NOT EXISTS Rented (" +
                    ...
                    "FOREIGN KEY (UID) REFERENCES User(UID))"; // "Listing" and "User" should be replaced with the actual table names of the Listing and User tables, respectively
