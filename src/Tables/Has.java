...
public class Has {
    ...
    public static void insert(Connection connection, int LID, int AID) {
        try (PreparedStatement statement = connection.prepareStatement("INSERT INTO Has (LID, AID) VALUES (?, ?)")) {
    ...
    public static Has selectById(Connection connection, int LID, int AID) {
        Has has = null;
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM Has WHERE LID = ? AND AID = ?")) {
            statement.setInt(1, LID);
            statement.setInt(2, AID);
    ...
    // Delete operation
    public void delete() {
        try (PreparedStatement statement = connection.prepareStatement("DELETE FROM Has WHERE LID = ? AND AID = ?")) {
    ...
    // Function to create the "Has" table if it doesn't exist (static approach)
    public static void createTable(Connection connection) {
        try (Statement statement = connection.createStatement()) {
            String sql = "CREATE TABLE IF NOT EXISTS Has (" +
                    "LID INT NOT NULL," +
                    "AID INT NOT NULL," +
                    "PRIMARY KEY (LID, AID)," +
            ...
            // of the Listing and Amenities tables, respectively
    ...
    // Function to drop the "Has" table if it exists (static approach)
